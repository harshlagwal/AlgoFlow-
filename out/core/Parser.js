...
class Parser {
            ...
            for (let index = 0; index < lines.length; index++) {
                ...
                // Function Definitions
                    ...
                    if (analyzedBlockName === 'Code Selection') {
                        ...
                        if (funcMatch)
                            analyzedBlockName = `function ${funcMatch[1]}`;
                    }
                }
                // Class Definitions
    ...
    static getImportExplanation(id, code) {
        const lang = id.toLowerCase();
        if (code.includes('math'))
            return 'Bringing in the **Math Toolbox** for complex calculations! 📐';
        if (code.includes('random'))
            return 'Bringing in the **Random Toolbox** to generate surprises! 🎲';
        if (code.includes('scanner') || code.includes('util'))
            return 'Bringing in **Utility Tools** to help with inputs and data! 🛠️';
        if (code.includes('iostream'))
            return 'Bringing in **Input/Output tools** for C++ to talk to you!';
        return 'Bringing in a **Box of Extra Tools** so the computer knows new tricks! 📦';
    }
    static getDefinitionExplanation(id, code) {
        const lang = id.toLowerCase();
        if (lang === 'python')
            return 'Using **def** to teach Python a new skill (function). We can call this later!';
        if (lang === 'java' || lang === 'cpp') {
            if (code.includes('main'))
                return 'This is the **Main Function**—the entry point where the adventure begins!';
    ...
    static getOutputExplanation(id) {
        switch (id.toLowerCase()) {
    ...
    static getStorageExplanation(id, code) {
    ...
    static getNodeComplexity(code, label, depth, allLines) {
        let time = 'O(1)';
        let space = 'O(1)';
    ...
    static refineExecutionOrder(nodes, edges, raw) {
        const trace = ['af_start'];
