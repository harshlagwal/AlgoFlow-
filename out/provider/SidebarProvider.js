...
class SidebarProvider {
    _extensionUri;
    static viewType = 'algoflow.sidebar';
    _view;
    ...
    resolveWebviewView(webviewView, context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case 'buildProject': {
                    await Scaffolder_1.Scaffolder.buildProject(data.projectType, data.mode);
                    break;
                }
            }
        });
    }
    async _copyContextForAI() {
...
*Algoflow Built for begineer*
    ...
    async _getDirectoryStructure(dir, depth = 0) {
        if (depth > 3)
