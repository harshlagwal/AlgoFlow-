...
import { DataBuilder } from '../core/DataBuilder';

export class FlowPanelProvider {
    public static currentPanel: FlowPanelProvider | undefined;
    ...
    private _lastCode: { text: string, languageId: string } | undefined;

    public update(code: string, languageId: string) {
        this._lastCode = { text: code, languageId };
        try {
            const graph = Parser.parse(code, languageId);
            const payload = DataBuilder.buildPayload(graph);
    ...
    private async _handleDownloadSVG(content: string) {
